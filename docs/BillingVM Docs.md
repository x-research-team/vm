...
- [Kernel Programming Language](#kernel-programming-language)
...
Kernel is a toy language interpreter, written in Go. It has C-style syntax, and is largely inspired by Ruby, Python, Perl and c#
...
This is a sample program using kernel language:
...
```sh
~ » kernel
Kernel programming language REPL
...
Kernel support two kinds of single line comment and also block comment.
...
Kernel supports 9 basic data types: `String`, `Int`, `UInt`, `Float`, `Bool`, `Array`, `Hash`, `Tuple` and `Nil`
...
In kernel, there are mainly eleven types of constants(Literals).
...
Variables in Kernel could start with the keyword `let`, or nothing with the
form `variable=value`.
...
Note：Starting from Kernel 5.0，when the decalared variable already exists, it's value will be overwritten:
...
In Kernel, you can use enum to define constants.
...
### Meta-Operators
Kernel has some meta-operators borrowed from perl6.
...
At the moment, Kernel has six meta-operators：
* <p>~+</p>
* <p>~-</p>
* <p>~*</p>
* <p>~/</p>
...
```
### using statement
In kernel, if you have some resources you want to release/free/close, e.g. close opened file, close network connection etc，
...
In kernel, you are free to define some operators, but you cannot
...
In kernel, integer is treated as an object, so you could call it's methods.
...
In kernel, float is also treated as an object, so you could call it's methods.
Please see below examples:
...
In kernel, decimal is Arbitrary-precision fixed-point decimal numbers.
...
In kernel, you could use [] to initialize an empty array:
...
In kernel, there are three types of `string`:
...
In kernel, strings are utf8-encoded, you could use utf-8 encoded name as a variable name.
...
### Hash
In kernel, the builtin hash will keep the order of keys when they are added to the hash, just like python's orderedDict.
...
In kernel, Hash is also an object, so you could use them to operate on hash object:
...
In kernel, `tuple` is just like array, but it could not be changed once it has been created.
...
### Extend basic type 
Kernel also provides support for extending basic types.
...
### Optional type
Kernel has support for Optional type like java8.
...
Kernel has limited support for the oop concept, below is a list of features:
...
The kernel parser could parse `public`, `private`, `protected`, but it has no effect in the evaluation phase.
That means kernel do not support access modifiers at present.
...
In kernel, all class is inherited from the root class `object`. 
...
Kernel has support for class `indexer`(like c#). 
...
Kernel also support class Category like objective-c（C# is called 'extension methods'）.
...
Kernel also has very simple annotation support like java：
...
Kernel also import some builtin annotations:
...
Maybe you are curious about why `nil` or `false` have message() function? Because in kernel, `nil` and `false`
...
In kernel, you could concatenate of different types. See below for examples:
...
Kernel support list(array,string, range, tuple) comprehensions.
...
Kernel also support hash comprehension.
...
Function in kernel is a first-class object. This means the language supports passing functions as arguments to
...
Before ver5.0, Kernel do not support multiple return values, But there are many ways to do it.
...
Starting from ver5.0, Kernel support multiple return values using 'let'.
...
Kernel has experimental support for working with `go` modules.

If you need to use go`s language package function, you first need to use `RegisterFunctions` or `RegisterVars` to
register `go` language functions or types into kernel language.

Below is an example of `main.go`(extracted):

```swift
// Because in kernel we already have built in module `fmt`, here we use `gfmt` for package name.
...
```
Now, in your kernel file, you could use it like below:
...
For more detailed examples, please see `goObj.ks`.

## Standard module introduction

In kernel, there are some standard modules provided for you. e.g. json, sql, sort, fmt, os, logger, time, flag, net, http, etc...
This is a brief introduction of some of the kernel standard modules, don't expect it to be thorough.
...
In kernel, the `linq` module support seven types of object:
...
Now, kernel has a powerful `linq for file` support. it can be used to operate
...
3. Recompile kernel source.

Below is a complete source of the `examples/db.ks`:
...
The formatter utility can format the kernel language.
The highlighter utility can highlight the kernel language to console or html.

You could also combine the two utilities:

```sh
./fmt xx.ks | ./highlight  //output to console(console highlight not support windows)
```
...
./mdoc -html -showsource -css 1 examples/doc.ks
...
./mdoc -html -showsource -css 1 -cssfile ./examples/github-markdown.css examples/doc.ks

//processing all the '.ks' files in examples directory, generate html.
./mdoc -html examples
```

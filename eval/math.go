...
func (m *Math) Round(line string, args ...Object) Object {
	if len(args) != 1 {
		panic(NewError(line, ARGUMENTERROR, "1", len(args)))
	}
	...
	case *Float:
		val = input
	default:
		panic(NewError(line, PARAMTYPEERROR, "first", "round", "*Integer|*UInteger|*Float", args[0].Type()))
	}

	return NewFloat(math.Round(val.Float64))
}

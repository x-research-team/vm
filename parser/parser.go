...
import (
	...
	"github.com/x-research-team/vm/ast"
	"github.com/x-research-team/vm/lexer"
	"github.com/x-research-team/vm/token"
)
...
func (p *Parser) ParseProgram() *ast.Program {
	...
	//if the kernel file only have ';', then we should return earlier.
...
func (p *Parser) getIncludedStatements(importpath string) (*ast.Program, error) {
	...
	fn := filepath.Join(path, importpath+".ks")
	...
	if err != nil { //error occurred, maybe the file do not exists.
		...
		if len(importRoot) == 0 { //'BILLING_ROOT' environment variable is not set
			return nil, fmt.Errorf("Syntax Error:%v- no file or directory: %s.ks, %s", p.curToken.Pos, importpath, path)
		} else {
			fn = filepath.Join(importRoot, importpath+".ks")
			e, err := ioutil.ReadFile(fn)
			if err != nil {
				return nil, fmt.Errorf("Syntax Error:%v- no file or directory: %s.ks, %s", p.curToken.Pos, importpath, importRoot)
			}
			f = e
		}
	}
